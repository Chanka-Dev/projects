        ...
        <div class="card-body">
            <form action="{{ route('pagos.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="monto_total" id="monto_total" class="form-control @error('monto_total') is-invalid @enderror" value="{{ old('monto_total') }}" step="0.01" required>
                    @error('monto_total')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="fecha_pago" id="fecha_pago" class="form-control @error('fecha_pago') is-invalid @enderror" value="{{ old('fecha_pago', now()->format('Y-m-d')) }}" required>
                    @error('fecha_pago')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
