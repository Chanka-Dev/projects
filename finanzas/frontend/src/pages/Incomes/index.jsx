...
export default function Incomes() {
  const { data: incomes, loading, refetch } = useApi(() => incomesApi.getAll())
  const { data: tags } = useApi(() => tagsApi.getAll({ type: 'income' }))
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing) await incomesApi.update(editing.id, form)
      else await incomesApi.create(form)
  ...
  const handleDelete = async () => {
    setSaving(true)
    try { await incomesApi.remove(deleting.id); setDeleting(null); refetch() }
    finally { setSaving(false) }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {!loading && !incomes?.length && <EmptyState icon="💰" message="No hay ingresos registrados" />}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {incomes.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-emerald-600 whitespace-nowrap">{formatCurrency(item.amount)}</td>
              ...
              <tfoot className="border-t-2 border-gray-200 bg-gray-50">
                <tr>
                  ...
                  <td className="px-4 py-3 text-right font-bold text-emerald-600">{formatCurrency(total)}</td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
            </table>
      ...
      <ConfirmDialog open={!!deleting} onClose={() => setDeleting(null)} onConfirm={handleDelete} loading={saving}
        ...
        message={`¿Eliminar este ingreso de ${formatCurrency(deleting?.amount)} del ${formatDate(deleting?.date)}?`} />
    </div>
  )
}
